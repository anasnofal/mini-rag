...
async def upload_data(
    ...
    is_valid, result_msg = data_controller.validate_uploaded_file(file=file)

    if not is_valid:

        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"result": is_valid, "result_msg": result_msg},
        )
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                "result": is_valid,
                "result_msg": ResponseResult.FILE_UPLOAD_FAILED.value,
            },
        )
    ...
    return JSONResponse(
        content={
            "result": is_valid,
            "result_msg": ResponseResult.FILE_UPLOAD_SUCCESS.value,
...
async def process_endpoint(
    ...
    if process_request.file_id:
        asset_record = await asset_model.get_asset_by_id_and_asset_name(
        ...
        if asset_record is None:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={
                    "signal": ResponseResult.FILE_ID_ERROR.value,
                },
            )
    ...
    if project_file_ids is None or len(project_file_ids) == 0:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"signal": ResponseResult.NO_FILES_FOUND.value},
        )
    ...
    for asset_id, file_id in project_file_ids.items():
        ...
        if file_content is None or len(file_content) == 0:
            logger.error(f"File content is empty for file_id: {file_id}")
            continue
        ...
        if file_chunks is None or len(file_chunks) == 0:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"signal": ResponseResult.PROCESSING_FAILED.value},
            )
        file_chunks_records = [
            DataChunk(
